...
weirdness on MANAWAVE's end.

Using MANAWAVE should feel like authoring HTML & CSS content regularly.
There's no special catch or exception. There's nothing special about
using MANAWAVE. It's just a marquee. It does everything you expect should happen.

Styling it with CSS works the same way as styling a `<div>` with a lot of content.
Writing HTML in a MANAWAVE marquee is the exact same as writing HTML regularly. There's no
syntactical exception or anything.

This means any existing style or content you have can be used in MANAWAVE without hassle.
If the content is responsive, MANAWAVE accomodates for that too.

## Customizable

MANAWAVE considers wacky use cases for MANAWAVE to behave beyond what a traditional
marquee should behave. You can see a bunch of these in the [Realms](/#realms).
...
intrude on your website. It shouldn't overwhelm nor break it. When the viewport resizes,
