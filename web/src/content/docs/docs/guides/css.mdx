...
If you plan on styling grids or flexboxes, you should make a wrapper element. Also, content that would be responsive without the marquee
...
:::caution
MANAWAVE measures content by its explicit dimensions ([box](https://developer.mozilla.org/en-US/docs/Learn/CSS/Building_blocks/The_box_model)) including `padding` and `margin`.
Because of this, `display: block` won't have the rest of its width if there's extra, and [margin collapsing](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_box_model/Mastering_margin_collapsing) doesn't work.
