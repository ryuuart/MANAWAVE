...
these methods, it could cause [FOUC](https://en.wikipedia.org/wiki/Flash_of_unstyled_content) because of [relayout](/docs/reference/behavior).
...
See also: [behavior of rendering marquee logic](/docs/reference/behavior)
...
Invoked when a layout or relayout occurs. This can override the final logical `size` and rendered `position` of
each [item](/docs/reference/behavior). For logical size, the DOM element still renders its original content and size.
...
See also: [behavior of layout](/docs/reference/behavior)
...
`onLoop` is invoked when an [item](/docs/reference/behavior) moves (wraps around) from one end or side of the marquee to the opposite.
...
`onElementCreated` is invoked when a marquee [item](/docs/reference/behavior) is rendered onto the marquee. You receive
...
`onElementDrawn` is invoked when a rendered marquee [item](/docs/reference/behavior) is re-rendered or updated. You receive an `element`
...
`onElementDestroyed` is invoked when a [relayout](/docs/reference/behavior) occurs. A relayout would basically cause the marquee to reset,
